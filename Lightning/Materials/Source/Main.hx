	...
	public override function render(context:RenderContext):Void {
		switch (context.type) {
			case OPENGL, OPENGLES, WEBGL:
				...
				lightColor.x = Math.sin(Timer.stamp() * 2.0);
				lightColor.y = Math.sin(Timer.stamp() * 0.7);
				lightColor.z = Math.sin(Timer.stamp() * 1.3);

				var diffuseColor = lightColor.crossProduct(new Vector4(0.5, 0.5, 0.5));
				var ambientColor = diffuseColor.crossProduct(new Vector4(0.2, 0.2, 0.2));
				...
				cube.render({
					...
					light: {
						ambient: [ambientColor.x, ambientColor.y, ambientColor.z],
						diffuse: [diffuseColor.x, diffuseColor.y, diffuseColor.z],
