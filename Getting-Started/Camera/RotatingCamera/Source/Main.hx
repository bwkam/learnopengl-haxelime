...
class Main extends Application {
	...
	var cameraTarget:Vector4;
	var cameraDir:Vector4;
	var cameraUp:Vector4;
	var cameraRight:Vector4;
	...
	public override function render(context:RenderContext):Void {
		switch (context.type) {
			case OPENGL, OPENGLES, WEBGL:
				...
				var radius = 10.0;
				var camX = Math.sin(Timer.stamp()) * radius;
				var camZ = Math.cos(Timer.stamp()) * radius;

				view = new Matrix4();
				view.identity();
				view.pointAt(new Vector4(0.0, 0.0, 0.0), new Vector4(camX, 1.0, camZ), new Vector4(0.0, 1.0, 0.0));
				proj = new Matrix4();

				proj.identity();
				createPerspectiveZO(proj, 45.0 * Math.PI / 180, Application.current.window.width / Application.current.window.height, 0.1, 100.0);
